...
package net.naidru.naidrumoreitemsmod.world.biome;
...
public class SkyBiomeBiome extends NaidruMoreItemsModModElements.ModElement {
	@ObjectHolder("naidru_more_items_mod:sky_biome")
	public static final CustomBiome biome = null;
	public SkyBiomeBiome(NaidruMoreItemsModModElements instance) {
		super(instance, 107);
	}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == CloudBlockBlock.block.getDefaultState().getBlock()
							|| ground == CloudBlockBlock.block.getDefaultState().getBlock())
							&& (ground2 == CloudBlockBlock.block.getDefaultState().getBlock()
									|| ground2 == CloudBlockBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), CloudBlockBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == CloudBlockBlock.block.getDefaultState().getBlock()
					|| blockType == CloudBlockBlock.block.getDefaultState().getBlock();
		}
